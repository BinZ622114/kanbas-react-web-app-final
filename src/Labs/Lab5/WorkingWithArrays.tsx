  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
        Update Completed</a>
      <input defaultValue={todo.id} className="form-control w-25 float-start me-4"
          onChange={(e) => setTodo({ ...todo, id: e.target.value })}/><span/>
      <input className="form-check-input float-start" type="checkbox" id="wd-array-completed"
          checked={todo.completed} onChange={(e) =>
            setTodo({...todo, completed: e.target.checked})}/>
      <label className="form-check-label" htmlFor="wd-array-completed">
            IsComplete
      </label><br /><hr />
